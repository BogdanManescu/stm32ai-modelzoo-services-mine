...
- The 'C:/ST/STEdgeAI_Core/4.x/scripts/N6_scripts/config_n6.json' file should be updated to configure the N6 loader.
```json
{
...
* `reid_distance_metric` : This is to set the distance metric to be used for the re-identification evaluation. It can be either `cosine` or `euclidean`.
...
* The `dataset_name` parameter should be appropriately set (example: 'DeepSportradar')
* The `test_query_path` and `test_gallery_path` must be set to the dataset folders used for the evaluation
...
evaluation:
   ...
   reid_distance_metric: cosine  # cosine / euclidean
